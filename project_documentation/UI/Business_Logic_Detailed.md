...
### 3.3. Order Cancellation Policy
*   **Endpoint**: `POST /api/v1/orders/orders/<id>/cancel/`
*   **Constraint**: Order status must be exactly `pending`.
*   **Transaction**: ACID transaction guarantees consistency during reversion.
*   **Steps**:
    1.  **Status Update**: Sets order status to `cancelled` and marks `datetime_cancelled`.
    2.  **Inventory Return**: Iterates through all items and restores `stock` to `ProductInStore` and `ProductInStoreInFlashsale` (if applicable), and decrements `sold_count`.
    3.  **Quota Restoration**: Adds the cancelled items' quantity back to the user's `daily_product_quota`.
    4.  **Voucher Reversal**: If a voucher was applied to the order, its corresponding `UserVoucherOrder` is marked as `is_used=False`.
    5.  **Membership & Point Reversal**: 
        *   Calculates the total points earned from the order via active `ProductInStorePoint` records.
        *   Deducts these points from the user's `UserMembership`.
        *   Evaluates if the user's points have fallen below their current tier's `min_point_earned` requirement and downgrades them if necessary.

---

## 4. Point & Membership Mechanisms

### 4.1. Point for Product Mechanism
*   **Logic**: During order checkout, the system identifies active `ProductInStorePoint` instances for the purchased items. Points are calculated per item (`point_earned * quantity`) and cumulatively awarded to the user's `UserMembership` upon order creation.

### 4.2. Point Claim Mechanism
*   **Endpoint**: `POST /api/v1/membership/user-point-rewards/`
*   **Logic**: Users can explicitly claim available `PointMembershipReward` items if they match their current membership level. Once claimed, a `UserPointMembershipReward` record is created, points are added to the user's total, and their `level_up_point` requirement is reduced. This triggers an evaluation for potential membership tier upgrades.

## 5. Address Management

### 5.1. Main Address Logic
