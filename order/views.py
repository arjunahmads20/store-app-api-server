...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def _validate_voucher(self, user, voucher_id, cart_items):
        ...
        # This uses product.sell_price * quantity as robust baseline (discounted price not considered)
        total_cost = 0
    ...
    def cancel(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            for item in order.products.all():
                ...
                if item.product_in_store_in_flashsale:
                    pis_flashsale = item.product_in_store_in_flashsale
                    pis_flashsale.stock += item.quantity
                    pis_flashsale.sold_count -= item.quantity
                    if pis_flashsale.sold_count < 0:
                        pis_flashsale.sold_count = 0
                    pis_flashsale.save()
                    
                if item.product_in_store_point:
                    total_points_to_deduct += item.product_in_store_point.point_earned * item.quantity
            ...
            if order_payment and order_payment.user_voucher_order:
                user_voucher = order_payment.user_voucher_order
            ...
            if total_points_to_deduct > 0:
                try:
                    ...
                    user_membership.deduct_point(total_points_to_deduct)
                    MembershipSystem.downgrade(user_membership)
                except UserMembership.DoesNotExist:
                    pass
                except Exception as e:
                    print(f"Error downgrading membership on cancel: {e}")
